<?php
// $Id: loisform_hooks.php,v 1.9.2.9 2010/10/17 21:52:38 quicksketch Exp $

/**
 * @file
 * Sample hooks demonstrating usage in Loisform.
 */

/**
 * @defgroup loisform_hooks Loisform Module Hooks
 * @{
 * Loisform's hooks enable other modules to intercept events within Loisform, such
 * as the completion of a submission or adding validation. Loisform's hooks also
 ...
 *       the module root.
 */
function hook_loisform_select_options_info() {
  $items = array();
...
/**
 * Respond to the loading of Loisform submissions.
 *
 * @param $submissions
 *   An array of Loisform submissions that are being loaded, keyed by the
 *   submission ID. Modifications to the submissions are done by reference.
 */
function hook_loisform_submission_load(&$submissions) {
...
/**
 * Modify a Loisform submission, prior to saving it in the database.
 *
 * @param $node
 *   The Loisform node on which this submission was made.
 * @param $submission
 *   The Loisform submission that is about to be saved to the database.
 */
function hook_loisform_submission_presave($node, &$submission) {
...
/**
 * Respond to a Loisform submission being inserted.
 *
 * Note that this hook is called after a submission has already been saved to
 * the database. If needing to modify the submission prior to insertion, use
 * hook_loisform_submission_presave().
 *
 * @param $node
 *   The Loisform node on which this submission was made.
 * @param $submission
 *   The Loisform submission that was just inserted into the database.
 */
function hook_loisform_submission_insert($node, $submission) {
  // Insert a record into a 3rd-party module table when a submission is added.
  db_query("INSERT INTO {mymodule_table} nid = %d, sid = %d, foo = '%s'", $node->nid, $submission->sid, 'foo_data');
}

/**
 * Respond to a Loisform submission being updated.
 *
 * Note that this hook is called after a submission has already been saved to
 ...
 *   The Loisform node on which this submission was made.
 * @param $submission
 *   The Loisform submission that was just updated in the database.
 */
function hook_loisform_submission_update($node, $submission) {
...
/**
 * Respond to a Loisform submission being deleted.
 *
 * @param $node
 *   The Loisform node on which this submission was made.
 * @param $submission
 *   The Loisform submission that was just deleted from the database.
 */
function hook_loisform_submission_delete($node, $submission) {
...
/**
 * Alter the display of a Loisform submission.
 *
 * This function applies to both e-mails sent by Loisform and normal display of
 * submissions when viewing through the adminsitrative interface.
 *
 * @param $renderable
 *   The Loisform submission in a renderable array, similar to FormAPI's
 ...
 *   rendered.
 */
function hook_loisform_submission_render_alter(&$renderable) {
...
/**
 * Modify a loaded Loisform component.
 *
 * IMPORTANT: This hook does not actually exist because components are loaded
 * in bulk as part of loisform_node_load(). Use hook_nodeapi() to modify loaded
 * components when the node is loaded. This example is provided merely to point
 * to hook_nodeapi().
 *
 * @see hook_nodeapi()
 * @see loisform_node_load()
 */
function hook_loisform_component_load() {
  // This hook does not exist. Instead use hook_nodeapi().
}

/**
 * Modify a Loisform component before it is saved to the database.
 *
 * Note that most of the time this hook is not necessary, because Loisform will
 ...
 *
 * @see hook_form_alter()
 * @see loisform_component_edit_form()
 *
 * @param $component
 *   The Loisform component being saved.
 */
function hook_loisform_component_presave(&$component) {
  $component['extra']['new_option'] = 'foo';
}

/**
 * Respond to a Loisform component being inserted into the database.
 */
function hook_loisform_component_insert($component) {
...
/**
 * Respond to a Loisform component being updated in the database.
 */
function hook_loisform_component_update($component) {
...
/**
 * Respond to a Loisform component being deleted.
 */
function hook_loisform_component_delete($component) {
 ...
 *
 *   _loisform_[callback]_[component]
 *
 ...
 * callbacks.
 *
 * @see loisform_component
 */
function hook_loisform_component_info() {
  $components = array();
...
/**
 * Alter the list of available Loisform components.
 *
 * @param $components
 *   A list of existing components as defined by hook_loisform_component_info().
 *
 * @see hook_loisform_component_info()
 */
function hook_loisform_component_info_alter(&$components) {
...
/**
 * @defgroup loisform_component Sample Loisform Component
 * @{
 ...
 * actual hooks, but instead samples of how Loisform integrates with its own
 * built-in components.
 */
 ...
 */
function _loisform_defaults_component() {
  return array(
    'name' => '',
    'form_key' => NULL,
    'mandatory' => 0,
 ...
 *
 * @param $component
 *   A Loisform component array.
 * @return
 *   An array of form items to be displayed on the edit component page
 */
function _loisform_edit_component($component) {
  $form = array();
  ...
  $form['extra']['options'] = array(
    ...
    '#description' => t('Key-value pairs may be entered separated by pipes. i.e. safe_key|Some readable option') . theme('loisform_token_help'),
    '#cols' => 60,
    '#rows' => 5,
    '#weight' => -3,
    '#required' => TRUE,
...
/**
 * Render a Loisform component to be part of a form.
 *
 * @param $component
 *   A Loisform component array.
 * @param $value
 ...
 */
function _loisform_render_component($component, $value = NULL) {
  $form_item = array(
    '#type' => 'textfield',
    '#title' => $component['name'],
    '#required' => $component['mandatory'],
    '#weight' => $component['weight'],
    '#description'   => _loisform_filter_descriptions($component['extra']['description']),
    '#default_value' => $component['value'],
    '#prefix' => '<div class="loisform-component-'. $component['type'] .'" id="loisform-component-'. $component['form_key'] .'">',
    '#suffix' => '</div>',
  );
 ...
 *   correlating to the loisform_submitted_data database table schema.
 * @param $format
 ...
 *    - #format
 *    - #value
 *   Loisform also uses #theme_wrappers to output the end result to the user,
 *   which will properly format the label and content for use within an e-mail
 *   (such as wrapping the text) or as HTML (ensuring consistent output).
 */
function _loisform_display_component($component, $value, $format = 'html') {
  return array(
    '#title' => $component['name'],
    '#weight' => $component['weight'],
    '#theme' => 'loisform_display_textfield',
    '#theme_wrappers' => $format == 'html' ? array('loisform_element') : array('loisform_element_text'),
 ...
 *
 * Note that Loisform will save the result of this function directly into the
 * database.
 *
 * @param $component
 *   A Loisform component array.
 ...
 */
function _loisform_submit_component($component, $value) {
 ...
 * @param $value
 *   An array of information containing the submission result, directly
 *   correlating to the loisform_submitted_data database schema.
 */
function _loisform_delete_component($component, $value) {
 ...
 *
 * This allows each Loisform component to add information into hook_help().
 */
function _loisform_help_component($section) {
  switch ($section) {
    case 'admin/settings/loisform#grid_description':
 ...
 *
 * This allows each Loisform component to add information into hook_theme().
 */
function _loisform_theme_component() {
  return array(
    'loisform_grid' => array(
      'arguments' => array('grid_element' => NULL),
    ),
    'loisform_mail_grid' => array(
 ...
 * 
 * This takes into account all submissions to this loisform. The output of this
 * function will be displayed under the "Results" tab then "Analysis".
 *
 * @param $component
 *   An array of information describing the component, directly correlating to
 *   the loisform_component database schema.
 * @param $sids
 ...
 *   submissions.
 */
function _loisform_analysis_component($component, $sids = array(), $single = FALSE) {
  // Generate the list of options and questions.
  $options = _loisform_component_options($component['extra']['options']);
  $questions = array_values(_loisform_component_options($component['extra']['questions']));
 ...
 */
function _loisform_table_component($component, $value) {
  $questions = array_values(_loisform_component_options($component['extra']['questions']));
  $output = '';
 ...
 */
function _loisform_csv_headers_component($component, $export_options) {
  $header = array();
  $header[0] = array('');
  $header[1] = array($component['name']);
  $items = _loisform_component_options($component['extra']['questions']);
  $count = 0;
 ...
 *   every row of data.
 */
function _loisform_csv_data_component($component, $export_options, $value) {
  $questions = array_keys(_loisform_select_options($component['extra']['questions']));
  $return = array();
