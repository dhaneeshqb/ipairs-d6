...
//	print '<br><br><pre>'.print_r($vars['node'],true).'</pre>';
  ...
  switch ($vars['node_type']) {
    case 'home':
     ...
     // $vars['home_slider'] = nyulmc_get_html_block_content('display_home_slider-block_1');
	  ...
	  //BEGIN--Interior Page with Slider, Damon Levine 10-15-2012//
	  case 'interior_slider':
      $vars['template_files'][] = 'page-node-type-interior_slider';
      $vars['styles'] = nyulmc_get_css('interior_slider');
      $vars['scripts'] = nyulmc_get_js('department_slider');
      $vars['department_slider'] = nyulmc_get_html_block_content('display_slider_department-block_1');
      break;
	  //END--Interior Page with Slider, Damon Levine 10-15-2012//
	  
	  //BEGIN--Interior Page with Right Sidebar and Slider, Damon Levine 12-6-2012//
	  case 'interior_right_sidebar_slider':
      $vars['template_files'][] = 'page-node-type-interior_right_sidebar_slider';
      $vars['styles'] = nyulmc_get_css('interior_right_sidebar_slider');
      $vars['scripts'] = nyulmc_get_js('department_slider');
      $vars['department_slider'] = nyulmc_get_html_block_content('display_slider_department-block_1');
      break;
	  //END--Interior Page with Right Sidebar and Slider, Damon Levine 12-6-2012// 
    ...
    case 'leadership':
    case 'people':
	case 'nyumc_event_import': //added this line to put left nav on Calendar Event Import content type--Damon Levine 12/20/11
	case 'blank_page': //added this line to create a page without sidebars for EPIC--Damon Levine 7/24/12
	...
	case 'fac_pub_interior_right_sidebar':
      $vars['template_files'][] = 'page-node-type-fac_pub_interior_right_sidebar';
	  $vars['column_main_class'] = 'column_main_feature';
      $vars['styles'] = nyulmc_get_css('fac_pub_interior_right_sidebar');
      $vars['scripts'] = nyulmc_get_js('interior_right_sidebar');
      	break;
		
     case 'faq_page':
	$vars['template_files'][] = 'page-node-type-interior';
	$vars['styles'] = nyulmc_get_css('faq_page');
	$vars['scripts'] = nyulmc_get_js('faq_page');
	break;
    case 'admin':
      $vars['styles'] = nyulmc_get_css('interior');
      break;
  ...
  $vars['section_title']  = nyulmc_get_section_title($vars['trail']);
	...
	$vars['section_name']   = nyulmc_get_section_name($vars['section_href'], $vars['section_title'], true);
  $vars['section_name2']   = nyulmc_get_section_name2($vars['section_href'], $vars['section_title']);//For the Header subhead 
...
//  $vars['main_nav']         = theme('links', menu_navigation_links($vars['node']->field_department_menu),  array('id' => 'nav_main',         'class'=>'menu'), TRUE, $vars['section_href'], $vars['trail']);
 ...
 // $vars['closure'] .= nyulmc_get_section_analytics();
  //kpr(get_defined_vars());
	...
	$head_fb  = '';	    
	$head_fb .= variable_get('og:title', '');
	$head_fb .= variable_get('og:description', '');
	$head_fb .= variable_get('og:image', '');
	$head_fb .= variable_get('og:url', '');
	$head_fb .= variable_get('og:site_name', '');
...
function nyulmc_preprocess_node(&$vars) {
	
	if ($vars['type'] == 'faq_page') {
		$qalist = array();
		$current_question = new stdClass();
		$index = 0;
		// structure the output differently so we can use jquery to expand the answers
		foreach ($vars['field_faq_question'] as $question) {
			$current_question->question = $question['value'];
			$current_question->answer = $vars['field_faq_answer'][$index]['value'];
			$current_question->index = $index;
			$qalist[] = clone $current_question;
			$index++;
		}
		$vars['qalist'] = $qalist;
	}

	if ($vars['type'] == 'qa_page') {
		$qalist = array();
		$current_question = new stdClass();
		$index = 0;
		// structure the output differently so we can use jquery to expand the answers
		foreach ($vars['field_qa_question'] as $question) {
			$current_question->question = $question['value'];
			$current_question->answer = $vars['field_qa_answer'][$index]['value'];
			$current_question->index = $index;
			$qalist[] = clone $current_question;
			$index++;
		}
...
function nyulmc_get_bucket_count($node) {
	$count = 0;
	$buckets = array();
	if ($node->type == 'home') {
		$buckets = $node->field_department_buckets;
	}
	elseif ($node->type == 'landing') {
		$buckets = $node->field_landing_buckets;
	}
...
function nyulmc_load_buckets($node) {
  $output = '';
  $position = 0;
  global $user;
  global $base_url;
  if ($node->type == 'home') {
	foreach ($node->field_department_buckets as $bucket) {
		$bid = $bucket['nid'];
		if ($bid == NULL) {
			continue;
		}
		...
		$img = theme('image', $bnode->field_bucket_image[0]['filepath']);
		$bnode->bucket_img = $img;
		if ($position==0) {
			$bnode->display_position = 'first';
		}
		$bnode->more_link = $bnode->field_bucket_link[0]['url'];
		$position++;
		$edit_link = $base_url . '/node/' . $bid . '/edit';
		$image_path = $base_url . '/' . drupal_get_path('theme', 'nyulmc') . '/images_layout/icon_pencil.png';
		if (in_array('webteam', array_values($user->roles)) || $user->uid==1) {
			$bnode->pencil = '<div class="edit-content-pencil"><a href="' . $edit_link . '"><img src="' . $image_path . '"/></a></div>';
		}
		$output .= theme('node', $bnode);
	}
  }
  elseif ($node->type == 'landing') {
	foreach ($node->field_landing_buckets as $bucket) {
		$bid = $bucket['nid'];
		if ($bid == NULL) {
			continue;
		}
  ...
  return check_markup($output,3,FALSE);
...
function nyulmc_get_js($custom) {
  ...
  switch ($custom) {
    ...
    case 'interior':
      ...
      break;
	case 'interior_slider'://BEGIN--Interior Page with Slider--Damon Levine 10-15-2012
      drupal_add_css(path_to_theme() .'/css/interior_slider.css', 'theme', 'all', FALSE);
      break;
	case 'interior_right_sidebar_slider'://BEGIN--Interior Page with Slider, Damon Levine 12-6-2012
      drupal_add_css(path_to_theme() .'/css/interior_right_sidebar_slider.css', 'theme', 'all', FALSE);
      break;  
    case 'nyumc_event_import': //added this line to put left nav on Calendar Event Import content type--Damon Levine 12/20/11
      drupal_add_js(path_to_theme() .'/js/leftnav_toggle.js');
      break;
    ...
    case 'direction':
      ...
      break;
    case 'faq_page':
	drupal_add_js(path_to_theme() .'/js/faq.js');
	drupal_add_js(path_to_theme() .'/js/leftnav_toggle.js');
	break;
  }
...
function nyulmc_get_css($custom = '', $extra = NULL) {
  ...
  switch ($custom) {
    ...
    case 'interior':
      ...
      drupal_add_css(path_to_theme() .'/css/sidebar_slider.css', 'theme', 'all', FALSE);
      break;
	case 'interior_slider'://Interior Page with Slider--Damon Levine 10-15-2012//
      drupal_add_css(path_to_theme() .'/css/interior_slider.css', 'theme', 'all', FALSE);
      break;
	case 'interior_right_sidebar_slider'://Interior Page with Right Sidebar and Slider, Damon Levine 12-6-2012//
      drupal_add_css(path_to_theme() .'/css/interior_right_sidebar_slider.css', 'theme', 'all', FALSE);
      break;  
	case 'nyumc_event_import': //added this line to put left nav on Calendar Event Import content type--Damon Levine 12/20/11
      drupal_add_css(path_to_theme() .'/css/interior.css', 'theme', 'all', FALSE);
    ...
    case 'interior_right_sidebar':
      ...
      break;	  
	case 'fac_pub_interior_right_sidebar':
      drupal_add_css(path_to_theme() .'/css/fac_pub_interior_right_sidebar.css', 'theme', 'all', FALSE);
      //drupal_add_css(path_to_theme() .'/css/landing.css', 'theme', 'all', FALSE);
      drupal_add_css(path_to_theme() .'/css/sidebar_slider.css', 'theme', 'all', FALSE);
      break;  
    case 'milestone_slider':
    ...
    case 'about_slider':
      ...
      break;
    case 'faq_page':
	drupal_add_css(path_to_theme() .'/css/faq.css', 'theme', 'all', FALSE);
	break;
  default:
...
**/
function nyulmc_get_section_name($href, $title, $url = false) {
	if ( $url ){
		return array('id' => 'site-name', 'body' => variable_get('site_name', ''));
	}
  return array('id' => 'site-name', 'body' => l(variable_get('site_name', ''), $variable ));
}

function nyulmc_get_section_name2($href, $title) {
	
  return array('id' => 'site-name2', 'body' => l(variable_get('site_name2', ''), variable_get('site_name2_url', '')));
}//For the Header subhead w/ URL 
...
function nyulmc_get_html_stylesie() {
  return  "\n"
    ...
    . '<link rel="apple-touch-icon" href="https://www.med.nyu.edu/sites/all/themes/nyulmc/images_layout/icon_mobile.png"/>' . "\n\n";
}
...
function nyulmc_get_html_alpha_clinical() {
  ...
  $return .= '<h2>Search Clinical Services by First Letter</h2>'."\n";
...
function nyulmc_get_html_alpha_directory() {
  ...
  $return  = '<div class="alpha-pager">'."\n";
  $return .= '<h2>Search Web Directory by First Letter</h2>'."\n";
...
function nyulmc_alter_menu_tree($attributes = array(), $node) {
  ...
  if ( ($menu_parent[1]['menu_name'] == 'menu-header-utility-nav') ||
	($active_menu == 'menu-header-utility-nav') ) {
