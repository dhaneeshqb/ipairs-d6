...
function nyulmc_set_breadcrumb($type = 'page', $parent = 'node/1', $current = 'node/1', $set_parent = FALSE) {
  ...
  if ($set_parent) {
    ...
    switch ($type) {
	  ...
	  case 'news':
	    $nodes = array(drupal_get_normal_path('media-relations'), drupal_get_normal_path('media-relations/news'));
			foreach ($nodes as $node) {
				$item = menu_get_item($node);
				if ($node != $current) {
					$title = $item['title'];
					if ($item['href'] == 'media-relations/news') { $title = 'News Releases'; }
					
					$trail[] = array('title' => t($title), 'href' => $item['href'], 'type' => $item['type'], 'crumb' => l(t($title), $item['href']));
				}
			}
			
			menu_set_active_item("media-relations/news");
			break;
		}
  }
...
/**
 * Modified implemetation of theme_pager
 */
 
function nyulmc_pager($tags = array(), $limit = 10, $element = 0, $parameters = array(), $quantity = 6) {
  ...
  $li_first = theme('pager_first', (isset($tags[0]) ? $tags[0] : t('&laquo; first')), $limit, $element, $parameters);
  $li_previous = theme('pager_previous', (isset($tags[1]) ? $tags[1] : t('&lsaquo; previous')), $limit, $element, 1, $parameters);
  $li_next = theme('pager_next', (isset($tags[3]) ? $tags[3] : t('next &rsaquo;')), $limit, $element, 1, $parameters);
  $li_last = theme('pager_last', (isset($tags[4]) ? $tags[4] : t('last &raquo;')), $limit, $element, $parameters);
...
function nyulmc_pager_current($text) {
	return "<a href=\"#\" class=\"bttn gray gray_active link\" onClick=\"void(0);\"><span>$text</span></a>";
}
...
function nyulmc_pager_link($text, $page_new, $element, $parameters = array(), $attributes = array()) {
  ...
  if (!isset($attributes['title'])) {
    ...
    if (!isset($titles)) {
      $titles = array(
        t('&laquo; first') => t('Go to first page'),
        t('&lsaquo; previous') => t('Go to previous page'),
        t('next &rsaquo;') => t('Go to next page'),
        t('last &raquo;') => t('Go to last page'),
      );
    }
